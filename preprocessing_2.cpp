...
int main(int argc, char** argv) {
    ...
    for (int i = 0; i < 15; i++)
        blur(imask, imask, Size(5, 5));

    cvtColor(imask, imask, CV_RGB2GRAY);
    adaptiveThreshold(imask, imask, 255, ADAPTIVE_THRESH_GAUSSIAN_C, THRESH_BINARY, 11, 2);
    string adaptive_output = path + "/adaptive" + ext;
    imwrite(adaptive_output, imask);
...
//    utils.imblur(src, imask);
    ...
    bool blob = false;
    Mat res;
    utils.fillBlobs(imask, imask, blob, NUMCORES);
    ...
    cout << "Finish writing blobs...." << endl;
