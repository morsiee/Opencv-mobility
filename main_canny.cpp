...
int main(int argc, char** argv) {
    ...
    //    bilateralFilter(cropped, contours, 15, 75, 75);
    //    blur(cropped, contours, Size(9, 9));

    medianBlur(cropped, imask, 9);
    ...
    //    string output = path + "/" + road + "-" + boost::lexical_cast<string>(thr) + ext;
    //    imwrite(output, contours);
    ...
    //    bitwise_and(cropped, contours, contours);

    //    fillPoly()
    //    namedWindow("Shape", WINDOW_AUTOSIZE);
    //    imshow("Shape", contours);
