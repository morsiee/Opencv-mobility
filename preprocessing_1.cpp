...
int main(int argc, char** argv) {
    ...
    for (size_t i = 0; i < lines.size(); i++) {
        Vec4i l = lines[i];
        line(edges, Point(l[0], l[1]), Point(l[2], l[3]), Scalar::all(255), 3, CV_AA);
    }
#endif

edges_output = path + "/edges with hough lines" + ext;
    imwrite(edges_output, edges);
#endif
...
#if 1
    utils.fillBlobs(edges, edges, NUMCORES);
    ...
    utils.iterative_imopen(res, 2, 25);

    string and_output = path + "/bitwise_and" + ext;
    imwrite(and_output, res);

    utils.fillBlobs(res, res, NUMCORES);
