 ...
 * g++ -O2 -std=c++11 -fopenmp main_blobs.cpp -o blobs.out `pkg-config --libs --cflags opencv` -I/usr/local/include/opencvblobslib -lopencvblobslib -lboost_filesystem -lboost_system
 */
...
int road_offset(Mat& src, vector<Point2d>& lvs, int thr, string path, int NUMCORES) {
    ...
    string blob_output = path + "/blob.png";
    ...
    return gps_line_shift(gps_lines, detected_lines_group, shift);
}
...
int main(int argc, char** argv) {
    ...
    Mat road = crop(src, lvs, bounding_box_point, tolerance);
    shift_coordinates(lvs, bounding_box_point.x, bounding_box_point.y);
    double sv = road_offset(road, lvs, thr, path, NUMCORES);
    ...
    plot_line(road, lvs, Scalar::all(255), 4);
    plot_line(road, nlvs, Scalar(0, 0, 255), 4);
